...
export function registerPeopleTools(server: McpServer, client: LegiScanClient) {
  ...
  server.tool(
    ...
    "Get all legislators active in a legislative session. Returns list of people with their roles, parties, and districts.",
    {
      session_id: z
        .number()
        .describe("Session ID from getSessionList"),
    },
    async ({ session_id }) => {
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(sessionPeople, null, 2),
            },
          ],
        };
      } catch (error) {
  ...
  server.tool(
    ...
    "Get all bills sponsored by a legislator across all sessions. Useful for researching a legislator's legislative priorities.",
    {
      people_id: z
        .number()
        .describe("People ID of the legislator"),
    },
    async ({ people_id }) => {
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: JSON.stringify(sponsoredBills, null, 2),
            },
          ],
        };
      } catch (error) {
